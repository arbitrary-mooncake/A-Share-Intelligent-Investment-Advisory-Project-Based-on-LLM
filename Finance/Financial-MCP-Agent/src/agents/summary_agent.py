...
async def summary_agent(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        **分析基准日期：{current_date}**
        ...
        ### 1. 核心结论
        [3-5句话：最核心的投资判断、关键理由、总体评级（使用定性表述如"积极关注/谨慎乐观/中性观望/谨慎回避"，禁止输出0-100数值综合评分）]
        ...
        ### 6. 短线 / 中线 / 长线判断
        [分别给出三个期限的专业判断，使用定性结论（如"短线看多但需警惕XX"/"中线中性偏谨慎"/"长线具备配置价值"），明确区分不同期限的逻辑和风险]
        [禁止给出各期限的数值评分，用专业判断文字替代]
        ...
        4. 数据不足时承认不确定性
        5. 使用简洁专业语言，避免空泛套话
        6. **禁止输出数值综合评分**：报告中不得出现 0-100 的综合打分或各期限数字评分（如"综合评分72分""短线65分"等）。各维度方向/置信度可保留（如"看多，置信度65%"），但总体和各期限结论必须是定性文字表述。
