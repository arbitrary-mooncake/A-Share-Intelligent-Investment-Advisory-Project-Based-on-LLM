...
def _build_system_prompt(template: str, current_date: str) -> str:
    ...
    base = (
        ...
        f"7. **时效性检查**：如果证据数据标注了「缓存数据」和获取时间，"
        f"必须检查该时间与当前基准时间 ({current_date}) 的差异。"
        f"对于行情/价格/成交量等时效性强的数据，超过1天的缓存数据必须在结论中标注"
        f"「基于{'{获取时间}'}的数据」，并在风险提示中说明数据滞后风险\n"
        f"8. **对话感**：像投研分析师在回答同事问题一样，自然流畅但不失专业"
    )
