...
{{
  ...
  "term_bias": "用 1 句话判断整个持仓偏向的期限投资风格（短期/中期/长期），并说明依据",
  "stock_advice": [
    {{
      "stock_code": "600519.SH",
      "company_name": "贵州茅台",
      "advice": "对该持仓的文字建议（不针对单只股票打分，而是描述性建议，如'权重偏高，建议适度减仓'或'基本面良好，可继续持有'）"
    }}
  ]
}}
```
...
term_bias 必须给出期限偏向判断。
stock_advice 为数组，每个持仓给出一条文字建议（不评分）。
"""
