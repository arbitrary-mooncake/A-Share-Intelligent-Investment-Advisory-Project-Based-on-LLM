...
# 每个 Tab 一个状态插槽（显示行正下方、密集列表上方），
# 页面底部的打分/报告轮询逻辑统一向对应插槽渲染，
# 让进度条出现在用户操作位置附近而不是页面最下端。
_tab_slots = {}
...
for tab, tk in zip(tabs, TERM_KEYS):
    with tab:
        ...
        # 状态插槽：打分/报告等异步任务的进度条渲染在这里
        _tab_slots[tk] = st.empty()
...
# ──────────────────────────────────────────────
# 精筛打分轮询（Tab 外，刷新安全；渲染到精筛 Tab 的状态插槽）
# ──────────────────────────────────────────────
for key in list(st.session_state.keys()):
    ...
    if not task_id:
        continue
    with (_tab_slots.get("fine") or st).container():
...
for code_key in list(st.session_state.keys()):
    ...
    with (_tab_slots.get(tk) or st).container():
...
# ──────────────────────────────────────────────
# 报告生成状态（Tab 外，避免切换丢失；渲染到精筛 Tab 的状态插槽）
# ──────────────────────────────────────────────
with (_tab_slots.get("fine") or st).container():
