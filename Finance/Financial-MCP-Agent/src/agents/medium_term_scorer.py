...
## 数据缺失时的表述规范
当分析数据不足时，必须使用分级表述：
- 核心数据完全缺失（如无任何财务/估值数据）：使用"**无法评估**"，评分不超过40分
- 关键数据部分缺失（如缺少估值数据但财务数据完整）：使用"**基于不完整数据**"，评分不超过65分
- 次要数据缺失（如缺少某项指标但整体判断不受影响）：使用"**存在数据缺口**"，正常评分但需标注
- 绝对禁止在关键数据缺失时给出"强烈推荐"或"减持"等强判断
"""
